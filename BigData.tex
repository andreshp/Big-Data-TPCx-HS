...
\section{Map Reduce. Hadoop} \label{sec:mr-hadoop}
	...
	\subsection{Map Reduce} \label{sec:mr-hadoop:map-reduce}
		...
		\begin{algorithm}
			...
			\begin{algorithmic}
				\State $key$: Palabra
				\State $values$: Conjunto con las ocurrencias de cada palabra
		...
		La paralelización  la lleva  cabo el sistema de forma automática. Para ello la entrada debe particionarse en $M$ bloques tiempo de ejecución. Cada uno de estos bloques contiene una lista de parejas (clave, valor). Map Reduce repartirá los bloques en distintos nodos del servidor de forma que cada uno de estos nodos ejecuta la función map sobre cada una de las parejas de su bloque. El programador debe indicar tanto el número de bloques a utilizar ($M$) como definir la función utilizada para el particionamiento de la entrada. Habitualmente se recomienda que cada uno de los $M$ bloques contenga entre $16$ y $64$ MB. Conforme los maps terminan de ejecutarse se va realizando la fase de filtrado u ordenación de los resultados. En este ejemplo concreto se obtiene para cada palabra el conjunto con todos los valores que se han emitido junto a esta. Una vez han finalizado los maps, se pueden ejecutar los reduces, también en paralelo, obteniendo como resultado la lista de palabras y el número de ocurrencias de las mismas.
